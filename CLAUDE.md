...
CI runs the unit suite on every push and PR to `master` against PHP 8.4 and 8.5 (matrix). See `.github/workflows/tests.yml`.

## Planned: PDO smoke-test Integration suite

`tests/Integration/` is declared in `phpunit.xml` but empty. Populate it with smoke tests that take SQL generated by each builder and `PDO::prepare()` it against an in-memory SQLite (zero-setup, runs in CI) and ideally MariaDB + Postgres containers. Catches three classes of bug that the unit suite cannot:

- **Class-not-found at render time** (the 0.10.0 → 0.10.1 `Rak200\SqlBuilder\Utils\Str` regression would have been caught here)
- **Invalid SQL that happens to assemble correctly as a string** (e.g. malformed `GROUPING SETS`, dialect-inappropriate keywords)
- **Cross-vendor divergence** (a feature gated by `UnsupportedFeatureException` for vendor X should actually parse on vendor Y)

Minimum scope: one test per top-level builder (`Select`, `Insert`, `Update`, `Delete`, `Merge`, `Set`, each DDL builder) that runs both `__toString()` and the `prepare(Dialect)` path through `PDO::prepare()`. Suite name `Integration` already wired in `phpunit.xml`. **Blocker for 1.0.0** — see the prerequisites discussion in CHANGELOG / commit history.
